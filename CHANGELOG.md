...
- Coverage for the rest of the framework, taking the suite to 96 files and 1524 tests, and the measured line coverage from 69% to 80%. Most of it is the surface the earlier suites skipped: the collection stack the concrete types inherit rather than declare (`Set`, `IndexPath`, `Dictionary` and `CollectionDifference` each cover their algebra or their semantics but not the container underneath), the predicate expression nodes and the selector-to-operator mapping, `ObjectClass`'s introspection and key-value machinery, and the networking types that carry no transport of their own. `HTTPURLProtocolServerTest` drives a real PHP development server for the redirect chain and the upload path, on a port derived from the process id so it never collides with `URLSessionTest`. What remains uncovered is either unreachable from a test — the POSIX branches of the user lookups, the symbolic-link operations on a Windows host, `is_hidden()`'s shell probe behind the `USE_UNSAFE_FUNCTIONS` constant — or needs a live transfer, an FTP server or a WebSocket server to reach; the suites that touch those say so in their docblocks rather than leaving the gap unexplained.
- `ext-gd` and `ext-simplexml` moved from `require` to `suggest`. Neither is needed to use the framework: `gd` is reached only by `Bundle::image()`, whose test already skips itself when the extension is absent, and `simplexml` appears in exactly one place in `src/` — the signature Psalm requires of the bundled plugin — so it is a development dependency that had been declared as a runtime one. Property lists are unaffected; they serialize through `ext-dom`, which stays required. The cost of the old declaration was borne by every consumer: a slim container without `gd` could not install the package even to use the collections.
- An `$allowedClasses` argument on `KeyedUnarchiver::unarchiveTopLevelObjectWithData()`, restricting which classes an archive may name. This is what secure coding is in PHP: `unserialize()` checks the list before it constructs anything, so a class outside it decodes to `__PHP_Incomplete_Class` and its constructor, `__wakeup()` and `__destruct()` never run. The framework this ports declares the guarantee per class through a marker protocol and states the expected class when decoding; PHP needs no equivalent, because the engine applies the restriction for the whole stream at the one point where decoding happens. The argument is optional and defaults to no restriction, since a caller reading back what it wrote itself is the common case — and because `CachedURLResponse::$userInfo` carries whatever the application put there, so a fixed list at that layer would silently decode an application's own class to an incomplete object. Pass a list when the data could have been written by anything other than this process.
- `CONVENTIONS.md`, a checklist of the mechanical conventions with the search that finds each violation — file layout, class and property rules, the collection idioms, when a comment earns its place, and what public API has to document. `CONTRIBUTING.md` and the README both link to it, since the analysers catch almost none of what it lists and a contributor who runs Psalm and PHPUnit and stops there has checked the smaller half. The rest of the stack points at the same document rather than copying it.
...
- Four Psalm suppressions that no longer suppressed anything. `ArgumentTypeCoercion` was declared twice with different file lists, and Psalm honours the last handler for an issue type, so the first block had never taken effect — nor had its entry for `Slice.php`, the one file it listed that the second did not. The other three (`HTTPCookieStorage.php` and `PredicateScanner.php` under `InvalidArgument`, `StandardAdditions.php` under `RedundantCondition`) covered code that has since been made type-safe. All 34 entries were checked by removing each one and re-running; the remaining 31 are still doing work.
