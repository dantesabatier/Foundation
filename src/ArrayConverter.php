...
final class ArrayConverter
{
    ...
    private(set) Dictionary $dictionary {
        get => $this->dictionary ??= new AssociativeArrayStrategy()->convert(is_sequential($this->reserved) ? array_combine(array_map(human_readable_value(...), array_keys($this->reserved)), array_values($this->reserved)) : $this->reserved);
    }
